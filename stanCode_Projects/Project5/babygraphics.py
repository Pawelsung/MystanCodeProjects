        ...
        if name in name_data:  # Check if name is in the data
            ...
            for j in range(len(YEARS)):   # Loop through each year
                ...
                if year in name_data[name]:  # Check if name has a rank for the year
                    ...
                    # canvas.create_line(x, y, x, y, width=LINE_WIDTH, fill=color)
            ...
            #     year1 = str(YEARS[j])
            #     year2 = str(YEARS[j+1])
            #     if year1 in name_data[name] and year2 in name_data[name]:
            #         rank1 = int(name_data[name][year1])
            #         rank2 = int(name_data[name][year2])
            #         y1 = GRAPH_MARGIN_SIZE + space_y * rank1
            #         y2 = GRAPH_MARGIN_SIZE + space_y * rank2
            ...
            #     elif year1 in name_data[name] and year2 not in name_data[name]:
            #         rank1 = int(name_data[name][year1])
            #         y1 = GRAPH_MARGIN_SIZE + space_y * rank1
            #         canvas.create_text(x1 + TEXT_DX, y1, text=f"{name} {rank1}", anchor=tkinter.SW)
            #     elif year1 not in name_data[name] and year2 in name_data[name]:
            #         rank2 = int(name_data[name][year2])
            #         y2 = GRAPH_MARGIN_SIZE + space_y * rank2
            ...
            #         canvas.create_text(x1+TEXT_DX, y, text=f'{name} {rank}', anchor=tkinter.SW, fill=color)
